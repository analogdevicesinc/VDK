...
#   $RCSfile$
#
#   Description: Makefile for compiling the BF547M
#
#   Last modified $Date$
...
ADSP-BF547M.ini:FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF547M.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF547M.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF547M_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
...
install: ADSP-BF547M.ini 
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${MKDIR} ${INSTALL_DIR}/lib
	${CP} VDK-BF547M.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF547M.ini ${INSTALL_DIR}/vdk
	
dist_clean:
	${RM} ${INSTALL_DIR}/ldf/VDK-BF547M.ldf
