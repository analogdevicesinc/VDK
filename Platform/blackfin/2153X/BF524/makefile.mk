...
#   $RCSfile$
#
#   Description: Makefile for compiling the BF524
#
#   Last modified $Date$
...
ADSP-BF524.ini:FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF524.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF524.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF524_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
...
install: ADSP-BF524.ini 
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${MKDIR} ${INSTALL_DIR}/lib
	${CP} VDK-BF524.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF524.ini ${INSTALL_DIR}/vdk
	
dist_clean:
	${RM} ${INSTALL_DIR}/ldf/VDK-BF524.ldf
