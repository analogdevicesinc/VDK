...
#   $RCSfile$
#
#   Description: Makefile for compiling the 21531
#
#   Last modified $Date$
...
ADSP-BF531.ini:FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF531.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF531.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF531_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@	
	${ECHO} "DefaultHistoryBufferSize = 100"		>> $@
	${DOS_ENDL}						>> $@	
...
install: ADSP-BF531.ini 
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${MKDIR} ${INSTALL_DIR}/lib
	${CP} VDK-BF531.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF531.ini ${INSTALL_DIR}/vdk
	
dist_clean:
	${RM} ${INSTALL_DIR}/ldf/VDK-BF531.ldf
	${RM} ${INSTALL_DIR}/vdk/ADSP-BF531.ini
	${RM} ${INT_VEC_SRCS}
