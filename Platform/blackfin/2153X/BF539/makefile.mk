...
#   $RCSfile$
#
#   Description: Makefile for compiling the 21539
#
#   Last modified $Date$
...
ADSP-BF539.ini:FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF539.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF539.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF539_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
...
install_debug: ADSP-BF539.ini 
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${MKDIR} ${INSTALL_DIR}/lib
	${CP} VDK-BF539.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF539.ini ${INSTALL_DIR}/vdk

install: install_debug
	
dist_clean:
	${RM} ${INSTALL_DIR}/ldf/VDK-BF539.ldf
