...
#   $RCSfile$
#
#   Description: Makefile for compiling the BF514
#
#   Last modified $Date$
...
ADSP-BF514.ini:FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF514.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF514.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF514_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
...
install: ADSP-BF514.ini 
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${MKDIR} ${INSTALL_DIR}/lib
	${CP} VDK-BF514.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF514.ini ${INSTALL_DIR}/vdk
	
dist_clean:
	${RM} ${INSTALL_DIR}/ldf/VDK-BF514.ldf
