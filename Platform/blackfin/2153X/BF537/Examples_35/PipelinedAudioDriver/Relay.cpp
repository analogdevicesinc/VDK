...
static void processBuffer(short vvSamples[][2], unsigned size)
{
	...
	//
	// We assume that flags 6-11 are already configured
	// as outputs (to the LEDs).
	//
	*pPORTFIO_CLEAR = ~bits & 0x0FC0; // clear flags
	*pPORTFIO_SET   = bits & 0x0FC0; // set flags
}
...
Relay::Run()
{
	...
	while (true)
    {
    	...
    	if (PCM_BUFFER_16 == typeOut && 100 == ++count)
    	{
    		processBuffer((short (*)[2])pPayloadOut, sizeOut);
    		count = 0;
    	}
