...
#   $RCSfile$
#
#   Description: Makefile for compiling the BF542
#
#   Last modified $Date$
...
ASM           = EASMBLKFN
ASMFLAGS      = -proc ADSP-BF542 ${UP_ASMFLAGS}
CXX           = ${UP_CXX}
CXXFLAGS      = -proc ADSP-BF542 ${UP_CXXFLAGS} ${INCLUDE_PATH} 
...
DEFINES               = ${UP_DEFINES} -DVERSION_PRODUCT_="VDK::kBF542"
endif
...
ifeq (${ADI_MAKE}, 1)
INCLUDE_PATH= -I '$(shell cygpath -w ${TOP_DIR}/blackfin/2153X/BF542)' ${UP_INCLUDE_PATH}
else
...
build_BF542: build_BF542_any 
clean: clean_current_dir clean_BF542_any 
install: install_examples install_BF542_any 
install_debug: install_examples install_debug_BF542_any 

# any libs
build_BF542_any:
	${MKDIR} bf542_rev_any
	"${MAKE}" -f../makefile.mk  -C bf542_rev_any build_libs ${NEW_FLAGS} VEROPTIONS="-si-revision any" UP_CXXFLAGS+="-si-revision any " UP_ASMFLAGS+="-si-revision any -D__WORKAROUNDS_ENABLED" NESTED_MAKE="YES"


clean_BF542_any:
	${MKDIR} bf542_rev_any
	"${MAKE}" -f../makefile.mk  -C bf542_rev_any clean_libs  NESTED_MAKE="YES"

install_BF542_any:
	"${MAKE}" -i -fmakefile.mk  install_libs ${NEW_FLAGS} SI_REV="bf542_rev_any" INST_DIR="bf542_rev_any"

install_debug_BF542_any:
	"${MAKE}" -i -fmakefile.mk  install_debug_libs ${NEW_FLAGS} SI_REV="bf542_rev_any" INST_DIR="bf542_rev_any"
...
ADSP-BF542.ini:FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF542.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF542.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF542_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
...
install_examples: ADSP-BF542.ini 
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${MKDIR} ${INSTALL_DIR}/lib
	${CP} VDK-BF542.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF542.ini ${INSTALL_DIR}/vdk
...
dist_clean:
	${RM} ${INSTALL_DIR}/ldf/VDK-BF542.ldf
