...
#   $RCSfile$
#
#   Description: Makefile for compiling the BF527
#
#   Last modified $Date$
...
VDKGEN_FLAGS = -proc ADSP-BF527 ++vdkgen_for_rt ++VDSP_5.0  ++install_dir '$(shell cygpath -w ${INSTALL_DIR}/..)'
else
ifeq (${VDSP_VERSION},4.5)
VDKGEN_FLAGS = -proc ADSP-BF527 ++vdkgen_for_rt ++VDSP_4.5  ++install_dir '$(shell cygpath -w ${INSTALL_DIR}/..)'
else
VDKGEN_FLAGS = -proc ADSP-BF527 ++vdkgen_for_rt ++VDSP_4.0  ++install_dir '$(shell cygpath -w ${INSTALL_DIR}/..)'
endif
endif
...
ADSP-BF527.ini:FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF527.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF527.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF527_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
...
install_debug: ADSP-BF527.ini ${VDKGEN}
	"${MAKE}" -fmakefile.mk -C../../../Utilities/VdkGen
	@echo "****VdkGen Built****"
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${MKDIR} ${INSTALL_DIR}/lib
	${CP} VDK-BF527.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF527.ini ${INSTALL_DIR}/vdk
	${CP} ${VDKGEN} ${INSTALL_DIR}/vdk
	@echo "**************Creating LDF Vdk.cpp and Vdk.h files for examples ***********"
	@echo ""
	${foreach example_dir,${EXAMPLES_LIST}, cp VDK-BF527.ldf ${example_dir};}
	${foreach example_dir,${EXAMPLES_LIST}, cp ../templates/ExceptionHandler-BF527.asm.tf ${example_dir}/ExceptionHandler-BF527.asm; }
	...
	@echo ""
	"${MAKE}" -i -f../../../../common/Examples/makefile.mk -C${EXAMPLES_DIR} UP_EXAMPLES_INSTALL_DIR=${INSTALL_DIR}/Examples/"No\ Hardware\ Required"/VDK/BF527 install
	# We don't want MultipleHeaps example anymore
	${RMDIR} ${INSTALL_DIR}/Examples/No\ Hardware\ Required/VDK/BF527/MultipleHeaps

install: install_debug

dist_clean:
	${RM} ${INSTALL_DIR}/ldf/VDK-BF527.ldf
	${RM} ${INSTALL_DIR}/vdk/ADSP-BF527.ini
...
squeaky: clean
	...
	${foreach example_dir,${EXAMPLES_LIST}, rm ${example_dir}/VDK-BF527.ldf; }
	${foreach example_dir,${EXAMPLES_LIST}, rm ${example_dir}/ExceptionHandler-BF527.asm; }
