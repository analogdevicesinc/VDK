...
#   $RCSfile$
#
#   Description: Makefile for compiling the BF506F
#
#   Last modified $Date$
...
ASM           = EASMBLKFN
ASMFLAGS      = -proc ADSP-BF506F ${UP_ASMFLAGS}
CXX           = ${UP_CXX}
CXXFLAGS      = -proc ADSP-BF506F ${UP_CXXFLAGS} ${INCLUDE_PATH} 
...
DEFINES               = ${UP_DEFINES} -DVERSION_PRODUCT_="VDK::kBF506F"
endif
...
PLATFORM    = ADSP-BF506F
VERS_DATE = $(shell date  +%a%b%e%Y%T")
OUTPUT                = ${OUT_DIR}BF532.dlb
ifeq (${ADI_MAKE}, 1)
INCLUDE_PATH= -I '$(shell cygpath -w ${TOP_DIR}/blackfin/2153X/BF506F)' ${UP_INCLUDE_PATH}
else
...
ifeq (${REVISION},all)
build_BF506F: build_BF506F_any
clean: clean_current_dir clean_BF506F_any
install: install_examples install_BF506F_any
install_debug: install_examples install_debug_BF506F_any
else
ifeq (${REVISION},any)
build_BF506F: build_BF506F_any 
clean: clean_current_dir clean_BF506F_any 
...
# any libs
build_BF506F_any:
	${MKDIR} bf506F_rev_any
	"${MAKE}" -f../makefile.mk  -C bf506F_rev_any build_libs ${NEW_FLAGS} VEROPTIONS="-si-revision any" UP_CXXFLAGS+="-si-revision any " UP_ASMFLAGS+="-si-revision any" NESTED_MAKE="YES"


clean_BF506F_any:
	${MKDIR} bf506F_rev_any
	"${MAKE}" -f../makefile.mk  -C bf506F_rev_any clean_libs  NESTED_MAKE="YES"

install_BF506F_any:
	"${MAKE}" -i -fmakefile.mk  install_libs ${NEW_FLAGS} SI_REV="bf506F_rev_any" INST_DIR="bf506F_rev_any"

install_debug_BF506F_any:
	"${MAKE}" -i -fmakefile.mk  install_debug_libs ${NEW_FLAGS} SI_REV="bf506F_rev_any" INST_DIR="bf506F_rev_any"
...
ADSP-BF506F.ini: FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF506F.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF506F.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF506F_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
...
install_examples: ADSP-BF506F.ini 
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${CP} VDK-BF506F.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF506F.ini ${INSTALL_DIR}/vdk
...
dist_clean:
	${RM} ${INSTALL_DIR}/ldf/VDK-BF506F.ldf
