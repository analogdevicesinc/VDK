...
#   $RCSfile$
#
#   Description: Makefile for compiling the BF523
#
#   Last modified $Date$
...
ASM                   = EASMBLKFN
ASMFLAGS      = -proc ADSP-BF523 ${UP_ASMFLAGS}
CXX                   = ${UP_CXX}
CXXFLAGS      = -proc ADSP-BF523 ${UP_CXXFLAGS} ${INCLUDE_PATH}
...
DEFINES               = ${UP_DEFINES} -DVERSION_PRODUCT_="VDK::kBF523"
endif
...
ifeq (${ADI_MAKE}, 1)
INCLUDE_PATH= -I '$(shell cygpath -w ${TOP_DIR}/blackfin/2153X/BF523)' ${UP_INCLUDE_PATH}
else
...
build_BF523: build_BF523_any 
clean: clean_current_dir clean_BF523_any 
install: install_examples install_BF523_any 
install_debug: install_examples install_debug_BF523_any 

# any libs
build_BF523_any:
	${MKDIR} BF523_rev_any
	"${MAKE}" -f../makefile.mk  -C bF523_rev_any build_libs ${NEW_FLAGS} VEROPTIONS="-si-revision any" UP_CXXFLAGS+="-si-revision any " UP_ASMFLAGS+="-si-revision any -D__WORKAROUNDS_ENABLED" NESTED_MAKE="YES"


clean_BF523_any:
	${MKDIR} bF523_rev_any
	"${MAKE}" -f../makefile.mk  -C bF523_rev_any clean_libs  NESTED_MAKE="YES"

install_BF523_any:
	"${MAKE}" -i -fmakefile.mk  install_libs ${NEW_FLAGS} SI_REV="bF523_rev_any" INST_DIR="bF523_rev_any"

install_debug_BF523_any:
	"${MAKE}" -i -fmakefile.mk  install_debug_libs ${NEW_FLAGS} SI_REV="bF523_rev_any" INST_DIR="bF523_rev_any"
...
ADSP-BF523.ini:FORCE
	...
	${DOS_ENDL}						>> $@
	${ECHO} "LinkerFile = $(QUOTE)blackfin\ldf\VDK-BF523.ldf$(QUOTE)"	>> $@
	${DOS_ENDL}						>> $@
	${ECHO} "Interrupts = $(QUOTE)$(CORE_INAMES)$(QUOTE)" >>$@
	${DOS_ENDL}						>> $@
	${ECHO} "ExceptionHandler = ExceptionHandler-BF523.asm" >>$@
	${DOS_ENDL}						>> $@
	...
	${DOS_ENDL}						>> $@
	${ECHO} "AsmIsrCodeTemplate = BF523_ISR_Asm_Source.asm"	>> $@
	${DOS_ENDL}						>> $@
...
install_examples: ADSP-BF523.ini 
	${MKDIR} ${INSTALL_DIR}/vdk
	${MKDIR} ${INSTALL_DIR}/ldf
	${CP} VDK-BF523.ldf ${INSTALL_DIR}/ldf
	${CP} ADSP-BF523.ini ${INSTALL_DIR}/vdk
