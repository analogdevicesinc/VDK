...
#   $RCSfile$
#
#   Description: Makefile for compiling the 21479
#
#   Last modified $Date$
...
QUOTE		= \"
DOS_ENDL   = @cmd /c echo.
CORE_VECS       = $(shell ${TYPE} ../../2146x/IntVectors.dat)
CORE_VECS2       = $(shell ${TYPE} ../../2146x/IntVectors2.dat)
...
ASMFLAGS	= ${UP_ASMFLAGS} -proc ADSP-21479 -file-attr ADI_OS=VDK -file-attr prefersMem=internal -file-attr prefersMemNum=30 -double-size-any -Wwarn-error
endif
CXX			= ${UP_CXX} 
CXXFLAGS	= -proc ADSP-21479 ${UP_CXXFLAGS} -double-size-any
...
CORE_VECS       = $(shell ${TYPE} ../../../2146x/IntVectors.dat)
CORE_VECS2      = $(shell ${TYPE} ../../../2146x/IntVectors2.dat)
TMR_INTERRUPTS  = $(shell ${TYPE} ../../../2146x/TimerInt.dat)
CORE_IRQNAME    = $(shell ${TYPE} ../../../2146x/IntIRQs.dat)
CORE_NOIRQNAME  = $(shell ${TYPE} ../../../2146x/IntNoIRQs.dat)
DEFINES		= ${UP_DEFINES}
else
CORE_VECS       = $(shell ${TYPE} ../../2146x/IntVectors.dat)
CORE_VECS2      = $(shell ${TYPE} ../../2146x/IntVectors2.dat)
...
CORE_IRQNAME    = $(shell ${TYPE} ../../2146x/IntIRQs.dat)
CORE_NOIRQNAME  = $(shell ${TYPE} ../../2146x/IntNoIRQs.dat)
DEFINES		= ${UP_DEFINES} -D VERSION_PRODUCT_="VDK::k21479" -DVDK_INT_=INT_INT14_P
endif
...
VPATH		= ${CURDIR}:${CURDIR}/../../2146x/21469:${UP_VPATH}
OUTPUT		= ${OUT_DIR}21479${SUFFIX}.dlb

ifeq (${ADI_MAKE}, 1)
INCLUDE_PATH= -I '$(shell cygpath -w ${TOP_DIR}/2146x/21479)' ${UP_INCLUDE_PATH}
else
...
VDKGEN_FLAGS = -proc ADSP-21479 ++vdkgen_for_rt ++VDSP_5.0 ++install_dir '$(shell cygpath -w ${INSTALL_DIR}/..)'
else
VDKGEN_FLAGS = -proc ADSP-21479 ++vdkgen_for_rt ++VDSP_4.5 ++install_dir '$(shell cygpath -w ${INSTALL_DIR}/..)'
endif
...
ifeq (${REVISION},all)
build_21479: asm_file_generation build_21479_any 
clean: clean_current_dir clean_21479_any 
install_libs: install_21479_any install_ldfs_and_examples
install_debug_libs: install_debug_21479_any 
else
ifeq (${REVISION},any)
build_21479: asm_file_generation build_21479_any
clean: clean_current_dir clean_21479_any
...
build_21479_any: 
	${MKDIR} 2147x_rev_any
	"${MAKE}" -f../makefile.mk  -C 2147x_rev_any build_21479_libs ${NEW_FLAGS} VEROPTIONS="-si-revision any" UP_CXXFLAGS+="-si-revision any" UP_ASMFLAGS+="-si-revision any" NESTED_MAKE="YES"

clean_21479_any:
	${MKDIR} 2147x_rev_any
	"${MAKE}" -f../makefile.mk  -C 2147x_rev_any clean_21479_libs ${NEW_FLAGS} NESTED_MAKE=YES

install_21479_any: 
	"${MAKE}" -i -fmakefile.mk  install_21479_libs ${NEW_FLAGS} SI_REV="2147x_rev_any" INST_DIR="21479_rev_any" 

install_debug_21479_any: 
	"${MAKE}" -i -fmakefile.mk  install_debug_21479_libs ${NEW_FLAGS} SI_REV="2147x_rev_any" INST_DIR="21479_rev_any" 

asm_file_generation: seg_int_hook.asm $(CORE_IRQS) $(CORE_NOIRQS) $(CORE_CB7I)

seg_int_hook.asm:	../../2146x/IntVectors.dat
	${ECHO} " .extern "	> $@
	${ECHO} " ${CORE_HLIST} ;" 			  >> $@
	${DOS_ENDL}	                                     >> $@
...
ADSP-21479.ini:FORCE
	...
	${DOS_ENDL}	                                     >> $@
	${ECHO} "LinkerFile = $(QUOTE)214xx\ldf\VDK-21479.ldf$(QUOTE)"	       >> $@
	${DOS_ENDL}	                                     >> $@
...
$(CORE_NOIRQS): ../../2146x/IntNoIRQs.dat
	${ECHO} "#include $(QUOTE)VDK_IntVector.h$(QUOTE)" > $@
	${DOS_ENDL}                                             >> $@
	${ECHO} " .file_attr Encoding=\"NW\"; "	>> $@
	${DOS_ENDL}                                             >> $@
...
squeaky_current_dir:
	...
	${foreach example_dir,${EXAMPLES_LIST}, rm ${example_dir}/VDK-21479.ldf; }
...
install_ldfs_and_examples: install_ldfs 

install_ldfs: ADSP-21479.ini ${VDKGEN}
	${MKDIR} ${INSTALL_DIR}/../214xx/ldf
	${MKDIR} ${INSTALL_DIR}/vdk
	${CP} VDK-21479.ldf ${INSTALL_DIR}/../214xx/ldf
	${CP} ADSP-21479.ini ${INSTALL_DIR}/../214xx/vdk
	${CP} ${VDKGEN} ${INSTALL_DIR}/vdk

install_debug_21479_libs: 
	${MKDIR} ${INSTALL_DIR}/lib/${INST_DIR}
	${CP} ${SI_REV}/debug/VDK-*-21469*.dlb ${INSTALL_DIR}/lib/${INST_DIR}

install_21479_libs:install_debug_21479_libs
	${CP} ${SI_REV}/release/VDK-*-21469*.dlb ${INSTALL_DIR}/lib/${INST_DIR}

dist_clean:
	"${MAKE}" -i -f../../../../common/examples/makefile.mk -C${EXAMPLES_DIR} UP_EXAMPLES_INSTALL_DIR=${INSTALL_DIR}/Examples/"No\ Hardware\ Required"/VDK/21479 dist_clean
	${RM} ${INSTALL_DIR}/ldf/VDK-21479.ldf
	${RM} ${INSTALL_DIR}/vdk/ADSP-21479.ini
