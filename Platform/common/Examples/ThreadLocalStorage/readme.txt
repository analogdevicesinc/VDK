Project Name: ThreadLocalStorage
Project Type: VDK
Hardware Used: None Required

Overview
--------
This example demonstrates how Thread Local Storage can be used to store a 
unique device descriptor for each instance of a thread within a VDK application.

The first thread which is run will allocate a thread slot in which the device 
descriptor can be stored, then each thread in turn will use this slot to open a 
device driver and store the resulting device descriptor.

Each of the threads can then call a global function, ReadWrite, which will
retrieve the  device descriptor from the current thread's slot and use this
descriptor to make calls to the Device Driver functions.

The threads in this example are all of an equal priority and are scheduled using
round robin scheduling on that priority level.
