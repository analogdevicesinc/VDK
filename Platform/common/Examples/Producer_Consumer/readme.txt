Project Name: Producer_Consumer
Project Type: VDK
Hardware Used: None Required

Overview
--------
An example of VDK's inter-thread messaging, showing most of the API functions 
(with the exception of DestroyMessage() and MessageAvailable()) and 
illustrating how messages may be re-used and forwarded.

It is based on the Producer-Consumer problem in the book "Operating Systems: 
...
The files included in this example are as follows:
	...
	+---Producer_Consumer
	    ...
	    ¦   Producer.h
            ...
            ¦
	    +---Kernel Files
	    ¦	Producer_Consumer.vdk
	    ¦	VDK.cpp
	    ¦	VDK.h

* Producer.cpp: A thread that acts as a random-number server, filling the 
                payload of each incoming empty message with a random number and
		returning it to its sender.

* Consumer.cpp: A thread which fetches random numbers from the producer and
                prints them.

* ProdCons.h:   A header file containing declarations of constants and
                enumerations which are required by both thread classes.
