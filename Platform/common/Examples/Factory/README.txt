Project Name: FactoryExample
Project Type: VDK
Hardware Used: None Required

Overview
--------
This simple example demonstrates an implementation of the design pattern known 
as a factory class in VDK. A factory class is used to create and initialize 
other threads, and allows the creation and initial conditions of many run-time 
objects to be controlled from a single piece of code.

The factory thread is created as a boot thread and runs as the first user code 
after kernel initialization. It spawns three other threads -- in this example 
ramp function generators -- that do the real work of the system.  As soon as it
creates these threads it destroys itself and frees its own system resources.

The factory thread creates three instances of the same thread class, but 
initializes them with different initial conditions, such that they generate 
three different signals. These initial conditions are passed to each of the
created threads via a pointer as part of the CreateThreadEx() API provided by
VDK.

This example also demonstrates that semaphores are not "owned" by a thread. Any
thread or ISR may post any semaphore at any time. When that happens, the thread
that pended on that semaphore first (if any) is moved to the ready state and is
made to run if it has the highest ready priority.
...
4) The status of the example can then be observed with the following mechanisms
   ...
   b) Using the VDK History window. 

Note that you will only get information in the VDK History window in the Debug
configuration or if VDK.cpp has been built with debug information on. This is
not enabled in the Release configuration by default.
