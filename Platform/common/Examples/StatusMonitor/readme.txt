Project Name: StatusMonitor
Project Type: VDK
Hardware Used: None Required

Overview
--------
This example demonstrates how VDK APIs can be used to retrieve the information 
that is displayed in the VDK Status window.

In this application the Status Monitor thread sits on top of a slightly modified
version of the VDK Producer Consumer example (modified to remove any printf 
statements), and will retrieve and store at a regular period of intervals a
number of bits of status information into a data structure. Selected information
from this data structure can then be printed out each time the data is
retrieved.
...
4) By placing a breakpoint on the call to PendSemaphore() within Status::Run() 
   will ensure that the information printed to the screen is easier to read,
   as the application will halt everytime the data has been retrieved and 
   printed.
